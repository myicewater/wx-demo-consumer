...
public class DateUtil {
	...
	public static Date string2TDate(String s) {
		if (StringUtil.isBlank(s)) {
			return null;
		}
		Date result = null;
	 ...
	 */
	public static int dateDiffNoTime(Date startDate, Date endDate) {
		DateTime start = new DateTime(startDate);
		DateTime end = new DateTime(endDate);
		return new Period(start , end , PeriodType.days()).getDays();//最后一个参数如果不写的话，下面的返回值将会是错误的。
	}
...
//		System.out.println(x+"天后是："+year+"-"+month+"-"+day);
	...
	public static void main(String[] args) {
		...
		System.out.println("当前日期：" + jdt.toString("YYYY-MM-DD"));
		System.out.println("当前时间：" + jdt.toString("YYYY-MM-DD hh:mm:ss"));
		System.out.println("当前天（也就是几号）：" + jdt.getDay());
		System.out.println("当前月：" + jdt.getMonth());
		System.out.println("当前年：" + jdt.getYear());
		System.out.println("当月天数：" + jdt.getMonthLength());
		System.out.println("当前天是当月的第几天：" + jdt.getDayOfMonth());
		System.out.println("当前天是当周的第几天：" + jdt.getDayOfWeek());
		System.out.println("当前天是当年的第几天：" + jdt.getDayOfYear());
		System.out.println("获取格式：" + jdt.getFormat());
		System.out.println("当前周是当月的第几周：" + jdt.getWeekOfMonth());
		System.out.println(jdt.getDateTimeStamp());
		
		System.out.println(jdt.getEra());
		System.out.println(jdt.getFirstDayOfWeek());
	}
	/**
	 * 获取两个时间相差多少秒
	 * @param beginTim
