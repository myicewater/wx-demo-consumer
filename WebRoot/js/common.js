var comUtil = {
		...
		showMask : function(){
			$.dialog({
				id:'_loadding',
				content : "<div style='height:19px;line-height:19px;width:220px;text-align:center;'><img src='"+ctx+"/images/loadding.gif'/></div>",
				padding : 0,
				title : false,
				resize: false,
				lock : true,
		...
		/** 弹出框-根据页面dom元素打开页面 */
		openUpload : function(_title, _dialogId, _width , _height ,_domId){
			$.dialog({
				title: _title,
				content: document.getElementById(_domId),
				id: _dialogId,
		...
		/** 
		 *把数组arr内的元素拼接成以指定type分隔的字符串（例如：aaa,bbb,cccs）
		 *arr:数组
		 *attr:如果数组内是对象形式，此参数则是指定对象的某个属性
		 *type:分隔符
		 **/
